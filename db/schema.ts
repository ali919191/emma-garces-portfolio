...
export const portfolioSettings = pgTable("portfolio_settings", {
  ...
  availabilityStatus: text("availability_status").$type<"available" | "limited" | "unavailable">().notNull().default("available"),
  primaryMarket: text("primary_market").notNull().default(""),
  travelAvailable: boolean("travel_available").notNull().default(true),
  additionalMarkets: text("additional_markets").notNull().default(""),
  availabilityNote: text("availability_note").notNull().default(""),
  compCardPrimaryMediaId: text("comp_card_primary_media_id").notNull().default(""),
  compCardMediaIds: jsonb("comp_card_media_ids").$type<string[]>().notNull().default([]),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
});
...
export const bookingInquiries = pgTable("booking_inquiries", {
  ...
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  status: text("status").$type<"new" | "reviewing" | "responded" | "booked" | "closed" | "spam">().notNull().default("new"),
  ...
  proposedDates: text("proposed_dates").notNull().default(""),
